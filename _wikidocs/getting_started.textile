...
*NOTE: The current release of Rudy (0.6) is still in alpha status. See the Alpha Notice in the README before continuing.*
...
You can install Rudy via Rubygems with the command below or download as a <a href="http://github.com/solutious/rudy/zipball/latest">tar</a> or <a href="http://github.com/solutious/rudy/zipball/latest">zip</a>. 
...
Rudy stores metadata about your machines and disks in SimpleDB. The following <tt>init</tt> command creates a SimpleDB domain called <tt>rudy_state</tt> to store this metadata.
...
We can now see that there is a new machine running:
...
<pre><code>  $ rudy -u root ssh</pre></code>
<pre><code>  Connecting root@ec2-11-22-33-44.compute-1.amazonaws.com m-us-east-1b-stage-app-01
...
<pre><code>  $ rudy shutdown</pre></code>
<pre><code>  All machines in stage-app will be shutdown and
  ...
  Detaching vol-fa0cef93... .done
  Destroying metadata... done
  
  ===  m-us-east-1b-stage-app-01  ec2-11-22-33-44.compute-1.amazonaws.com 
  Destroying m-us-east-1b-stage-app-01
  
  No instances running</code></pre>

h2. Built-in Help

The Rudy command line tools have plenty of built-in documentation. 
